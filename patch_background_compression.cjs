const fs = require('fs');
let code = fs.readFileSync('src/components/BackgroundManager.tsx', 'utf8');
...
if (regex.test(code)) {
  ...
  fs.writeFileSync('src/components/BackgroundManager.tsx', code);
  console.log("Patched processFile to compress images in BackgroundManager.");
} else {
  console.log("Failed to find processFile regex in BackgroundManager.");
}
